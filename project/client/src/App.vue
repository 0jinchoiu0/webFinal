    ...
    <section class="section-block appointment-form">
      <div class="form-grid">
        ...
        <form @submit.prevent="submitAppointment">
          ...
          <label v-else>
            預約諮商服務
            <input type="text" disabled value="初次諮商，可由中心安排諮商師" />
          </label>
          <label v-if="isFollowUp">
            諮商方式
      ...
      <div class="appointment-result" v-if="appointmentResult">
        ...
        <p><strong>諮商師：</strong>{{ appointmentResult.counselor_name || '待安排' }}</p>
        <p><strong>類型：</strong>{{ appointmentResult.appointment_type }}</p>
        <p><strong>方式：</strong>{{ appointmentResult.mode || '待安排' }}</p>
